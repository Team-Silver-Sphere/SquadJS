...
export default class DiscordStats extends BasePlugin {
  static get description() {
    return 'Basic Discord bot to display player stats';
  }
  ...
  async onMessage(message) {
    ...
    switch (cmd) {
      case 'link': {
        ...
        if (
          this.linkedAccounts.find(
            (x) => x.steamID === steamID || message.author.id === x.discordID
          )
        )
          return message.reply('Account already linked');
      ...
      case 'kd': {
        ...
        topPlayers.forEach((entry, index) => {
          ...
          leaderboardEmbed.addField(
            `#${index + 1}`,
            `Player: ${entry.username} (${entry.steamID})\nKD Ratio: ${kdRatio}`
          );
        });
  ...
  async showHelp(message) {
    const helpEmbed = new Discord.MessageEmbed()
      ...
      .addFields(
        ...
        {
          name: '!leaderboard',
          value: 'View the top 3 players with the most kills on the GE server.'
        }
      );
  ...
  async getUserStats(user) {
    ...
    const mostKilledPlayer = await this.models.Death.findOne({
      ...
      attributes: [
        'victim',
        [Sequelize.fn('COUNT', Sequelize.col('victim')), 'count'],
        'victimName'
      ],
    ...
    const mostDiedPlayer = await this.models.Death.findOne({
      ...
      attributes: [
        'attacker',
        [Sequelize.fn('COUNT', Sequelize.col('attacker')), 'count'],
        'attackerName'
      ],
    ...
    const content = new Discord.MessageEmbed()
      ...
      .setThumbnail(
        `https://cdn.discordapp.com/attachments/1212650237173309511/1212650591583477820/GOL...jpg?ex=65f29bde&is=65e026de&hm=36a67da42ee2cd95122aff6fbb4dd47e8f691d4d829454ab866a1fec584288b4&`
      )
      .addFields(
        { name: 'Kills', value: kills, inline: true },
        { name: 'Deaths', value: deaths, inline: true },
        { name: 'Team Kills', value: tks, inline: true },
        { name: 'KD', value: kdr(2), inline: true },
        { name: 'Revive', value: revives, inline: true },
        { name: 'Revives Per Death', value: rpd, inline: true },
        ...
        {
          name: 'Played Match',
          value: matcheCount ? matcheCount.dataValues.count : 0,
          inline: true
        },
        { name: '\u200B', value: '\u200B', inline: true },
        { name: '\u200B', value: '\u200B', inline: true },
        ...
        {
          name: 'Most Killed',
          value: mostKilledPlayer
            ? `${mostKilledPlayer.victimName} (${mostKilledPlayer.dataValues.count})`
            : 'N/A',
          inline: true
        },
        {
          name: 'Most Died',
          value: mostDiedPlayer
            ? `${mostDiedPlayer.attackerName} (${mostDiedPlayer.dataValues.count})`
            : 'N/A',
          inline: true
        },
        {
          name: 'Most Revived',
          value: mostRevivedPlayer
            ? `${mostRevivedPlayer.victimName} (${mostRevivedPlayer.dataValues.count})`
            : 'N/A',
          inline: true
        }
      )
